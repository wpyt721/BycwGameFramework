...
public class CreatUISourceUtil {
    ...
    public const string OutPutPath = "/Game/Scripts/UIControllers/";
    //创建UISource文件的函数
    public static void CreatUISourceFile(GameObject selectGameObject)
    {
        ...
        if (File.Exists(Application.dataPath + OutPutPath + className + ".cs")) {
            return;
        }

        sw = new StreamWriter(Application.dataPath + OutPutPath + className + ".cs");
        sw.WriteLine(
        ...
        Debug.Log("Gen: " + Application.dataPath + OutPutPath + className + ".cs");
    }
}
